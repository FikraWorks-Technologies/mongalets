...
type Action = { icon: any; title: string; text: string; onClick?: () => void }

const serviceCards = [
  { title: 'Electrical & repairs', text: 'Electricians, plumbers, carpenters and appliance technicians.', image: 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?auto=format&fit=crop&w=900&q=85' },
...
function RoleDashboard({ role, roleName, session, actions, onInvite, onReport, onNavigate }: { role: Role; roleName: string; session: Session; actions: Action[]; onInvite: () => void; onReport: () => void; onNavigate: (label: string) => void }) {
  if (role === 'owner' || role === 'admin') return <ManagementDashboard role={role} roleName={roleName} actions={actions} onNavigate={onNavigate} />
  if (role === 'tenant') return <TenantDashboard session={session} actions={actions} onInvite={onInvite} onReport={onReport} onNavigate={onNavigate} />
  if (role === 'provider') return <ProviderDashboard session={session} actions={actions} onNavigate={onNavigate} />
...
function TenantDashboard({ session, actions, onInvite, onReport, onNavigate }: { session: Session; actions: Action[]; onInvite: () => void; onReport: () => void; onNavigate: (label: string) => void }) {
  return <><DashboardHeading eyebrow="Resident · Marina Bay · A5G" title={`Welcome home, ${session.name.split(' ')[0]}`} text="Your space for visitors, reports and trusted services — without building-wide security information." action={<><button className="button button--secondary" onClick={onReport}><Icon name="camera" size={16} /> Report issue</button><button className="button button--primary" onClick={onInvite}><Icon name="key" size={16} /> Invite visitor</button></>} /><section className="tenant-hero"><div><span className="eyebrow">Your home</span><h2>Building A · Floor 5 · Unit A5G</h2><p>Access, visitor invitations and service requests are kept in one simple place.</p><div className="tenant-status"><span><Icon name="shield" size={15} /> Secure</span><span><Icon name="check" size={15} /> Profile verified</span></div></div><img src={propertyImage} alt="Residential home interior" /></section><QuickActions actions={actions} /><section className="security-dashboard-grid tenant-grid"><article className="security-panel"><div className="security-panel-head"><div><small>Your access</small><h2>Upcoming visitors</h2></div><button className="security-text-link" onClick={() => onNavigate('Visitors')}>View all <Icon name="arrow-right" size={14} /></button></div><div className="visitor-mini"><div><span><Icon name="users" size={16} /></span><div><strong>Grace W.</strong><small>Today · 2:30 PM · Approved</small></div></div><b>Approved</b></div><div className="visitor-mini"><div><span><Icon name="tool" size={16} /></span><div><strong>DenzeK Services</strong><small>Today · 4:00 PM · Service visit</small></div></div><b>Expected</b></div></article><article className="security-panel"><div className="security-panel-head"><div><small>Request status</small><h2>My latest report</h2></div><button className="security-text-link" onClick={() => onNavigate('My reports')}>My reports <Icon name="arrow-right" size={14} /></button></div><div className="request-timeline"><div className="is-done"><span><Icon name="check" size={13} /></span><div><strong>Plumbing issue</strong><small>Provider assigned · Visit scheduled for today</small></div></div><div><span><Icon name="clock" size={13} /></span><div><strong>Completion confirmation</strong><small>You'll be asked to confirm the repair.</small></div></div></div></article></section><ServicePreview onNavigate={onNavigate} /></>
}

function ProviderDashboard({ session, actions, onNavigate }: { session: Session; actions: Action[]; onNavigate: (label: string) => void }) {
  return <><DashboardHeading eyebrow="Service provider · Your workspace" title={`Good morning, ${session.name.split(' ')[0]}`} text="Manage your jobs, scheduled visits and building access — without seeing resident or building-wide presence data." action={<button className="button button--primary" onClick={() => onNavigate('Schedule')}><Icon name="calendar" size={16} /> View schedule</button>} /><section className="security-metric-grid role-metrics provider-metrics"><article><span><Icon name="tool" size={19} /></span><small>Open jobs</small><strong>3</strong><p>Assigned to you</p></article><article><span><Icon name="calendar" size={19} /></span><small>Today's visits</small><strong>2</strong><p>Scheduled jobs</p></article><article><span><Icon name="check" size={19} /></span><small>Completed</small><strong>18</strong><p>This month</p></article><article><span><Icon name="key" size={19} /></span><small>Access visits</small><strong>1</strong><p>Next at 4:00 PM</p></article></section><section className="provider-hero"><div><span className="eyebrow">Next service visit</span><h2>Plumbing repair · Unit A5G</h2><p>Today at 4:00 PM · Resident-approved service request</p><div className="provider-tags"><span>Access approved</span><span>Photo attached</span></div></div><img src={serviceCards[0].image} alt="Professional repair service" /></section><QuickActions actions={actions} /><section className="security-panel jobs-panel"><div className="security-panel-head"><div><small>Your work queue</small><h2>Upcoming jobs</h2></div><button className="security-text-link" onClick={() => onNavigate('My jobs')}>All jobs <Icon name="arrow-right" size={14} /></button></div><div className="job-row"><span><Icon name="tool" size={17} /></span><div><strong>Electrical inspection</strong><small>Tomorrow · Unit B-112</small></div><b>Scheduled</b></div><div className="job-row"><span><Icon name="tool" size={17} /></span><div><strong>Deep cleaning</strong><small>Friday · Unit C-301</small></div><b>Pending access</b></div></section></>
}

function GuestDashboard({ session, actions, onNavigate }: { session: Session; actions: Action[]; onNavigate: (label: string) => void }) {
  return <><DashboardHeading eyebrow="Guest · Approved visit" title={`Welcome, ${session.name.split(' ')[0]}`} text="Everything you need for your visit. You only see your invitation, access instructions and useful services." /><section className="guest-pass"><div className="guest-pass-main"><span className="eyebrow">Visitor pass</span><h2>Grace W. · Unit A5G</h2><p>Today · 2:30 PM – 6:00 PM</p><div className="guest-pass-code"><span><Icon name="check" size={18} /></span><div><strong>Access approved</strong><small>Present this pass to security when you arrive.</small></div></div></div><img src={propertyImage} alt="Residential building entrance" /></section><QuickActions actions={actions} /><section className="guest-info-grid"><article className="security-panel"><span className="feature-icon"><Icon name="shield" size={20} /></span><small>Access guide</small><h2>Arrive with confidence.</h2><p>Show your visitor pass at the security desk. Follow the property's visitor instructions and leave through the designated exit.</p></article><article className="security-panel"><span className="feature-icon"><Icon name="briefcase" size={20} /></span><small>Useful services</small><h2>Need a ride or help?</h2><p>Browse available cabs, cleaning, repair and other services without accessing private resident information.</p><button className="security-text-link" onClick={() => onNavigate('Services')}>Browse services <Icon name="arrow-right" size={14} /></button></article></section></>
}

function ServicePreview({ onNavigate }: { onNavigate: (label: string) => void }) {
  return <section className="security-panel service-preview-panel"><div className="security-panel-head"><div><small>Resident convenience</small><h2>Trusted services</h2></div><button className="security-text-link" onClick={() => onNavigate('Services')}>View all <Icon name="arrow-right" size={14} /></button></div><div className="security-service-grid">{serviceCards.map((service) => <button key={service.title} className="security-service-card" onClick={() => onNavigate('Services')}><img src={service.image} alt="" /><div><strong>{service.title}</strong><small>{service.text}</small></div></button>)}</div></section>
}
